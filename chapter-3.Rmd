...
5. What happens if you make a scatterplot of ```class``` and ```drv```? Why is the plot not useful?
    ...
    The plot isn't useful as a scatterplot, because it is mapping two categorical variables against one another on a plot that is intended to represent quantitative scale -- the differences between steps have no quantitative meaning. (Note: Is this right/are there other reasons as well?)
...
d1 +
  geom_point(mapping = aes(), shape = 21, color = 'white')
```
